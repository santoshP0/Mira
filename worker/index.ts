 ...
 *
 * Push notifications are sent via Expo's push API — no FCM server key needed.
 * App must call Notifications.getExpoPushTokenAsync() and store the result.
 */
...
async function runEscalations(env: Env) {
  ...
  // because alertFamily sets status to 'escalated' before the 30-min threshold.
  ...
  // Generate today's dose_logs around midnight (UTC 00:00–00:05)
...
async function processDose(dose: DoseLogRow, now: Date, env: Env) {
  ...
  // Each branch is gated by status to prevent duplicate escalation:
  // - 'pending'   → nudge (T+10) then family alert (T+15, sets 'escalated')
  // - 'escalated' → mark missed (T+30)
  if (minsLate >= MARK_MISSED_MIN && dose.status === 'escalated') {
    await markMissed(dose, env);
  } else if (minsLate >= FAMILY_ALERT_MIN && dose.status === 'pending') {
    await alertFamily(dose, env);
  } else if (minsLate >= SECOND_NUDGE_MIN && dose.status === 'pending') {
...
async function supabaseQuery<T>(env: Env, path: string): Promise<T[]> {
  const url = `${env.SUPABASE_URL}${path.trim()}`;
...
// Supports batching up to 100 messages per request.
async function sendPush(
  ...
  const messages = tokens.map((token) => ({
    ...
    priority: 'high',
    channelId: notification.channelId ?? 'mira-reminders',
  }));
